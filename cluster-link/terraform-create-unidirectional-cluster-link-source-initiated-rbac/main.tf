...
//Este cluster link será criado APÓS o destination ser criado como INBOUND.
resource "confluent_cluster_link" "source_to_destination_outbound" {
  ...
  destination_kafka_cluster {
    id                 = data.confluent_kafka_cluster.destination_cluster.id
    bootstrap_endpoint = data.confluent_kafka_cluster.destination_cluster.bootstrap_endpoint
    credentials {
  ...
  depends_on = [
    confluent_cluster_link.destination_to_source_inbound
  ]
}
//Este clusterlink fica oculto, para fins de parametrização
resource "confluent_cluster_link" "destination_to_source_inbound" {
  link_name       = var.cluster_link_name
...
resource "confluent_kafka_mirror_topic" "mirror_topic_source" {
  ...
  cluster_link {
    link_name = confluent_cluster_link.source_to_destination_outbound.link_name
  }
  kafka_cluster {
    id            = data.confluent_kafka_cluster.destination_cluster.id
    rest_endpoint = data.confluent_kafka_cluster.destination_cluster.rest_endpoint
  ...
  depends_on = [
    confluent_cluster_link.source_to_destination_outbound
  ]
}